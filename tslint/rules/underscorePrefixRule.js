...
UnderscorePrefixWalker.prototype.visitMethodDeclaration = function(node) {
  ...
  if (variableName[0] !== '_' && (modifiers.indexOf('private') !== -1 || modifiers.indexOf('protected') !== -1)) {
    ...
    this.addFailure(
      this.createFailure(node.getStart(), node.getWidth(), "'" + variableName + "' private/protected methods need a '_' prefix")
    );
  }
...
UnderscorePrefixWalker.prototype.visitPropertyDeclaration = function(node) {
  ...
  if (variableName[0] !== '_' && (modifiers.indexOf('private') !== -1 || modifiers.indexOf('protected') !== -1)) {
    // create a failure at the current position
    this.addFailure(
      this.createFailure(node.getStart(), node.getWidth(), "'" + variableName + "' private/protected properties need a '_' prefix")
    );
  }
