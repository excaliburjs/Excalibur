...
```typescript
const attributeCallback = (attData: AttributeData) => {
...
```typescript
const spawnEntities = (attData: AttributeData) => {
  const { tileData, mapData } = attData;
...
const spriteFusionMap = new SpriteFusionResource({
  ...
  spritesheetPath: './map/spritesheet.png',
  attributeCallback: spawnEntities,
...
const map = new SpriteFusionResource({
  mapPath: './map/map.json',
  spritesheetPath: './map/spritesheet.png',
  attributeCallback: yourCallback,
...
- **5 tiles, one source of truth** — no more coordinate synchronization
