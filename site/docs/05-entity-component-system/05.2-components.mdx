...
{@apilink Component | `Components`} are a useful tool for storing data associated with an entity. Components can be used with {@apilink System | `Systems`} to implement complex behavior. Sometimes {@apilink Component | `Components`} can be a better choice than using inheritance. Components provide reusable behavior that can be easier to maintain over complex inheritance hierarchies.
