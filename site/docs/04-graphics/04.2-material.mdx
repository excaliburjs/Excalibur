...
void main(void) {
    ...
    // if you aren't using the ex.glsl tag you must manually premultiply
    // or if you're introducing your own opacity
...
To update a material's uniforms we recommend using [[Material.update]], the callback is passed a [[MaterialContext]] containing the material's declarative `uniforms` dictionary — the same dictionary as `material.uniforms`. Setting values there is the recommended way to change uniforms, since they reach the composite shader and, when a pipeline is used, are forwarded to every pass on the next draw. For the rarer case of calling shader methods directly, `context.shaders` and `context.shadersByName` give access to every compiled [[Shader]] the material drives.
...
* `vec2 u_graphic_resolution` - Current graphic's resolution (Note: if this is a sprite sheet it's the whole sheet)

* `vec2 u_size` - Current destination draw size (think the final game world size)

* `mat4 u_matrix` - Excalibur's current orthographic projection matrix

* `mat4 u_transform` - Excalibur's current world transform

* `sampler2D u_graphic` - Current graphic's texture (Note: if this is a sprite sheet it's the whole sheet, `v_uv` will give you the slice)
