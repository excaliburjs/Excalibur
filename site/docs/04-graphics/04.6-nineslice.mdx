...
export type NineSliceConfig = GraphicOptions & {
  ...
  */
  ...
  /*
  sourceConfig has all the parameters necessary to cut up the input text properly, width and height are the overal pixel dimensions of the texture and the margins define the overall cutting boundaries in pixels
  */
...
note: these properties are merged with GraphicsOptions, so it is strongly recommended to provide you're overall width and height properties in conjunction with what's listed here. They are optional properties on the Graphics Options, but there are implications if they are not specified.
...
There are two parameters in the configuration that need to be set to dictate the behavior of the drawing desitnation graphic, `stretchH` and `stretchV`. The are of type `NineSliceStretch`. The `stretchH` parameter will dictate the behavior of tile slices 2,5, and 8, as these frames can stretch horizontally. The `stretchV` parameter dictates how tile slices 4,5,6 behave, as they can stretch vertically.
