...
You can set parameters under `sourceConfig` properties to control the margins. This lets you dial in where the comde 'slices' your texture up. For example, with the example 64x64 sprite, we can set the top/bottom margins to 5 pixels, and the left/right margins to 6 pixels to get this slicing.
