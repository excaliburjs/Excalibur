...
Under normal circumstances this is fantastic for breaking cache when assets change. However Tiled assumes its dependent files are located in the same relative directory, using the same name that was used locally.
