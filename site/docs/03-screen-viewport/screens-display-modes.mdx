...
The screen-space origin `(0, 0)` (the [[CoordPlane.Screen]] frame) is the top-left of the safe content area, not the raw canvas top-left. [[Screen.contentAreaOffset]] holds the resolution-space position of that origin — for example, in a horizontal-clip scenario (window wider than the content aspect ratio):
...
Vertical-clip (window taller than the content aspect ratio) is analogous on the Y axis. [[CoordPlane.Screen]] entities (`ScreenElement`s) drawn at `pos = (0, 0)` appear at the content area's top-left on screen.
...
Behaves identically to [[DisplayMode.FitScreenAndFill]] above (same C-frame rooting, same offset math) but driven by the parent element's size instead of the window.
...
By default `enterFullscreen()` will use the canvas element as the root of full screen mode. If your game uses [HTML based UI](/docs/html), the HTML UI will not be included because it is not a child of the canvas element.
