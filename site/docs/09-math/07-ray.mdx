...
[[Vector.normalize]], which guards the exact-zero case and returns [[Vector.Zero]]
...
The real silent-NaN risk isn't in [[Ray]] — it's in [[LineSegment.getSlope]], which
...
Since [[Ray.intersect]] calls `line.getSlope()` internally, **passing a degenerate
