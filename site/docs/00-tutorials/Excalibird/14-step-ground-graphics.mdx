...
:::tip
Excalibur also ships a [[TiledSprite]] graphic that handles the destination size and wrapping for you —
see the [Tiled Sprite guide](/docs/tiled-sprite) for a higher-level alternative to the manual
`sourceView.width` + `destSize.width` approach shown below.
:::
