...
class HealthSystem extends System {
  ...
  update(delta: number) {
    for (const entity of this.query.entities(this.types)) {  //<--- LOOP THROUGH ALL ENTITIES FOR THIS SYSTEM
...
player.get(HealthComponent).damage(60); // triggers system → player dies
