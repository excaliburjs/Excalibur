...
[[Scene.physics|scene.physics]] exposes two `query` overloads for finding colliders in the current physics
world. Both return the live [[Collider]] instances — you can read their owning entity, shape, or bounds from
the result.

### Query by Point

[[PhysicsWorld.query|`physics.query(point: Vector)`]] returns every [[Collider]] that **contains** the
point. Useful for click-to-select, pointer picking, or "what's under this position" checks.

```typescript
const colliders = game.currentScene.physics.query(ex.vec(150, 200));
...
[[PhysicsWorld.query|`physics.query(bounds: BoundingBox)`]] returns every [[Collider]] whose **bounds
overlap** the supplied [[BoundingBox]]. Useful for selection-box picks, "is anything inside this region"
checks, or pre-broadphase lookups.

```typescript
const region = new ex.BoundingBox({ left: 100, top: 100, right: 300, bottom: 250 });
const colliders = game.currentScene.physics.query(region);
```

:::note
These are point/bounds overlap queries against the collider's [[Collider.bounds|world bounds]]. For
ray-based queries (line-of-sight, ground-detection sweeps), see [[PhysicsWorld.rayCast|`scene.physics
.rayCast`]] instead.
:::
