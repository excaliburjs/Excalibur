...
[[Scene.physics]] exposes two `query` overloads for finding colliders in the current physics
...
:::note
These are point/bounds overlap queries against the collider's [[Collider.bounds]]. For
ray-based queries (line-of-sight, ground-detection sweeps), see [[PhysicsWorld.rayCast]]
(`scene.physics.rayCast`) instead.
:::
