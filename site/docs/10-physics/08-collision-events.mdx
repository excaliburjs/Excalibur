...
The **collisionstart** event is fired when a {@apilink Body | `physics body`}, usually attached to an actor, first starts colliding with another {@apilink Body | `body`}, and will not fire again while in contact until the the pair separates and collides again.
...
The **collisionend** event is fired when two {@apilink Body | `physics bodies`} are no longer in contact. This event will not fire again until another collision and separation.
