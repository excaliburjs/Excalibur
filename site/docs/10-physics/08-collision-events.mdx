...
The **collisionstart** event is fired when a [[Body|physics body]], usually attached to an actor, first starts colliding with another [[Body|body]], and will not fire again while in contact until the pair separates and collides again.
...
:::tip
If your actor sticks on tile seams while walking across a solid tilemap, see
[Solving Tile-Seam Sticking](/docs/tilemap#solving-tile-seam-sticking) for the `contactSolveBias` option.
:::
