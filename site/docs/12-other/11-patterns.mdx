...
Where possible, we use `onInitialize()` for initialization logic instead of the constructor. This saves CPU cycles because it's called before the first update an entity is needed and makes it easy to restart a game or reuse something by manually calling `onInitialize()`.
