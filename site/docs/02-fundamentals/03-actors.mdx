...
Actors can be nested within another, and all children have their transform (position, rotation, scale, z) relative to their parent. This is useful for a number of use cases.
...
You can override the core internal `Actor.update` but it is important to call the `super.update(...)` method in order 
to preserve the Excalibur core update implementation. You do this at your own peril, please use the [[Actor.onPreUpdate]]/[[Actor.onPostUpdate]]
...
Override the [[Actor.onPreUpdate]] method to update the state of your actor before the internal `Actor.update`.
...
[[Actor.onPostUpdate]] is called after the internal `Actor.update` to prepare state for the _next_ frame. Things that need to be updated include state, drawing, and position.
...
In order for it to be drawn it must be converted to a [Sprite](/docs/sprites). A common pattern is to define graphics in the `onInitialize` method of an actor.
...
[[GraphicsComponent.onPreDraw|Actor.graphics.onPreDraw]] is an optional function-property hook that fires
just before the actor's graphic is drawn. The [[ExcaliburGraphicsContext]] at this point is in **entity-local
space** — the camera and the actor's own (and any ancestor) transforms have already been applied, so `(0,0)`
is the actor's world position. Don't translate to your actor's `pos` yourself; it's done for you.

:::note

`onPreDraw` fires inside the `GraphicsSystem`'s per-entity save/restore scope, so anything you `save()` you
must also `restore()` to avoid leaking transforms into sibling actors.
...
class Player extends ex.Actor {
  constructor(config?: ex.ActorArgs) {
    ...
    this.graphics.onPreDraw = (ctx: ex.ExcaliburGraphicsContext, elapsed: number) => {
      // Draw before the actor's graphic — entity-local space
      ctx.drawRectangle(ex.vec(0, 0), 8, 8, ex.Color.Yellow);
    };
  }
}
```
...
[[GraphicsComponent.onPostDraw|Actor.graphics.onPostDraw]] is an optional function-property hook that fires
just after the actor's graphic is drawn, **in the same save/restore scope** as `onPreDraw`. It is the
recommended hook for custom drawing because the actor's graphic has already been rendered (so you can draw
overlays, debug indicators, etc. on top).
...
class Player extends ex.Actor {
  constructor(config?: ex.ActorArgs) {
    ...
    this.graphics.onPostDraw = (ctx: ex.ExcaliburGraphicsContext, elapsed: number) => {
      // Draw after the actor's graphic — entity-local space
      ctx.drawRectangle(ex.vec(0, 0), 4, 4, ex.Color.Red);
    };
  }
}
...
For a full breakdown of every draw-phase hook, its coordinate space, and where it sits in the per-frame
timeline, see the [Lifecycle Events reference](/docs/lifecycle-events#draw-phase-coordinate-spaces).
