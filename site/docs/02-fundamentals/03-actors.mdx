...
{@apilink Actor | `Actors`} are prebuilt Excalibur {@apilink Entity | `Entities`} that come with the batteries included for most applications.
Such as {@apilink TransformComponent | `position`}, {@apilink MotionComponent | `velocity`}, {@apilink GraphicsComponent | `graphics`}, and {@apilink ColliderComponent | `collision detection`}.

{@apilink Actor | `Actors`} are the recommended way to use Excalibur to show something on the screen.
