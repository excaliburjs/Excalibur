...
If [[Engine]] is loading, the entire Update section is skipped and only [[DefaultLoader.onUpdate]] (the
loader's `onUpdate`) runs; in Draw, the loader canvas is rendered instead of the scene.
...
If [[PhysicsConfig.substep]] is greater than 1, `precollision`, `postcollision`,
...
- **Deferred entity removal**: [[Entity.kill]] (the `entity.kill()` call) sets `isActive = false`; the actual array
...
- **Pointer events fire in z-order**: when an event targets overlapping actors, higher-z actors get it
  first. Call [[PointerEvent.cancel]] in a handler to make lower-z actors skip the
