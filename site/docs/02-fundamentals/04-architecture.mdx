...
the "scene graph" which is the {@apilink Scene | `scenes`} and the tree of {@apilink Actor | `actors`} within that
...
For complex games, this is recommended for {@apilink Actor | `actors`} and {@apilink Scene | `scenes`}, especially.
