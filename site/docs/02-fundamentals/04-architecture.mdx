...
a [[Loader]] which you can use to [load assets](/docs/loader) like sprites and sounds.
...
You would include your script (or [bundle it](/docs/installation)) on an HTML page and that page does _not need anything else_.
...
Engines also accept options like [[EngineOptions.antialiasing|antialiasing]], [[EngineOptions.snapToPixel|
snapToPixel]], and [[EngineOptions.maxFps|maxFps]] that control pixel rendering and frame-rate caps. See the
[Engine Options reference](/docs/engine-options) for the full list.

## Lifecycle events

Excalibur exposes `onX` override hooks on [[Engine]], [[Scene]], [[Actor]]/[[Entity]], [[Camera]],
[[DefaultLoader]], and [[Transition]], plus per-class event subscriptions like `engine.on('preupdate', ...)`.

For a complete enumeration of every override hook, every event-name subscription, the per-frame timing order,
and the graphics-context coordinate space at each draw hook, see the
[Lifecycle Events reference](/docs/lifecycle-events).
...
Call [[Engine.toggleDebug]], or [[Engine.showDebug]] with a `true` or `false` to enable or disable Excalibur's debug feature. This will enable [actor debug drawing](/docs/actors) to help diagnose drawing issues.
