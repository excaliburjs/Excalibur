...
Nearly everything in Excalibur has a way to listen to events! This is useful when you want to know exactly when things happen in Excalibur and respond to them with game logic. {@apilink Actor | `Actors`}, {@apilink Scene | `Scenes`}, {@apilink Engine | `Engines`}, {@apilink ActionsComponent | `Actions`}, {@apilink Animation | `Animations`}, and various components have events you can hook into just look for the `.events` member!
