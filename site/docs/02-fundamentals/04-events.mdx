...
For a complete enumeration of every engine/scene/actor/camera lifecycle hook and event subscription
name with timing and coordinate-space details, see the [Lifecycle Events reference](/docs/lifecycle-events).
...
Excalibur has types on all its event listeners, you can check these types with Intellisense in VS Code or by following the Typescript definition.
