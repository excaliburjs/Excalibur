...
[[Entity.kill|`entity.kill()`]] does **not** immediately remove the entity. It flips an `isActive` flag to
...
:::note
The deferred entity is still iterated by systems for the remainder of the **current** frame — its
`isActive` flag flips to `false` only after the kill call, so systems that already started processing it
this frame will finish. It is removed before the next update.
:::
...
Actors extend the same pattern: [[Actor.kill|`actor.kill()`]] wraps `super.kill()` with `_prekill` and
