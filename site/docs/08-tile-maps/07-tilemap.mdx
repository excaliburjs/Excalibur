...
{@apilink TileMap | `Tilemaps`} consist of a uniform grid of cells that can be solid or not. Each cell can have it's own graphics.

Tile maps are made up of {@apilink Tile | `Tiles`} which can draw {@apilink Graphic | `Graphics`}. Tile
...
and export them to JSON. You can then load them using a {@apilink Resource | `Generic Resource`}
