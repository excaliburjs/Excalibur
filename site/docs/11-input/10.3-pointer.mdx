...
There is always at least one {@apilink PointerAbstraction | `Pointer`} available ([[Input.PointerEventReceiver.primary|engine.input.pointers.primary]]) and
...
{@apilink Actor | `Actors`} can handle pointer events targeted to them. By default, the collision geometry is used to test whether the pointer is
