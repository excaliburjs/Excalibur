---
title: Gamepads and Controllers
slug: /gamepad
section: Input
---
...
You must opt-in to controller support ([[Input.Gamepads.enabled|engine.inputs.gamepads.enabled]]) because it is a polling-based
...
HTML5 Gamepad API only supports a maximum of 4 gamepads. You can access them using the [[Input.Gamepads.at|engine.input.gamepads.at]] method. If a [[Input.Gamepad|Gamepad]] is
...
{@apilink Buttons | `Gamepad buttons`} typically have values between 0 and 1, however depending on
...
{@apilink Axes | `Gamepad axes`} typically have values between -1 and 1, but even idle
