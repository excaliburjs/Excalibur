...
If looking to have a more full fledged stack, we recommend the [T3](https://create.t3.gg/) stack which creates and setups up a new NextJS project with an ORM ([Prisma](https://www.prisma.io/) or [Drizzle](https://orm.drizzle.team/)), [TailwindCSS](https://tailwindcss.com/), an Auth Provider ([NextAuth / AuthJS](https://next-auth.js.org/) or [Clerk](https://clerk.com/docs)) and [tRPC](https://trpc.io/docs/) for backend full stack typesafety. To do so, use the command:
