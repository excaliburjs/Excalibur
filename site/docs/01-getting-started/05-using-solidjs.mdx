...
If looking to have a more full fledged stack, we recommend the [Create JD App](https://github.com/OrJDev/create-jd-app?tab=readme-ov-file) command which creates and setups up a new SoldJS project with a similar stack to that of the [T3](https://create.t3.gg/) stack. It provides an ORM ([Prisma](https://www.prisma.io/)), [TailwindCSS](https://tailwindcss.com/), an Auth Provider ([NextAuth / AuthJS](https://next-auth.js.org/)) and [pRPC](https://github.com/solidjs-community/mediakit/tree/main/packages/prpc) for backend full stack typesafety. To do so, use the command:
