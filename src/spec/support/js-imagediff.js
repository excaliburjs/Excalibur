...
(function(name, definition) {
    ...
    toImageEqual: function(util, customEqualityTesters) {
      return {
        compare: function(actual, expected) {
          var div = get('div'),
            a = get('div', '<div>Expected:</div>'),
            b = get('div', '<div>Actual:</div>'),
          ...
          if (result.pass) {
            result.message =
              'Expected: ' +
              newline +
              actualCanvas.toDataURL('image/png', 0) +
              doubleline +
              'To be: ' +
              newline +
              expectedCanvas.toDataURL('image/png', 0) +
              doubleline +
              'Diff: ' +
              newline +
              can.toDataURL('image/png', 0); //div;
          } else {
            result.message =
              'Expected: ' +
              newline +
  ...
  imagediff = {
    ...
    expectImageMatches: function(src, image, done) {
      var a = new Image();
      expect.bind(this);
      a.addEventListener('load', function() {
        expect(image).toImageEqual(a);
        copyCanvas = null;
        copyCtx = null;
        done();
      });
