...
export class RigidBodySolver {
  ...
  preSolve(contacts: CollisionContact[]) {
    ...
    const finishedContactIds = Array.from(this.idToContactPoints.keys());
    for (const contact of contacts) {
      ...
      const contactPoints = this.idToContactPoints.get(contact.id) ?? [];
    ...
    if (Physics.warmStart) {
      ...
      for (const contact of contacts) {
        const contactPoints = this.getContactPoints(contact.id);
  ...
  warmStart(contacts: CollisionContact[]) {
    for (const contact of contacts) {
      ...
      for (const point of contactPoints) {
        ...
        const tangentImpulse = contact.tangent.scale(point.tangentImpulse).scale(0.2);
