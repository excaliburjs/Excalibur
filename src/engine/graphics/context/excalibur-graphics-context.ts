...
export interface AntialiasOptions {
   ...
   *
   * Default {@link ImageFiltering.Blended}
   */
...
export interface ExcaliburGraphicsContext {
   ...
   *
   * To force a specific draw call order, use {@link ExcaliburGraphicsContext.z}
   *
   ...
   *
   * Note it is important to all {@link ExcaliburGraphicsContext.save} and {@link ExcaliburGraphicsContext.restore} when modifying state.
   */
  z: number;
  ...
  /**
   * Set the background color of the graphics context, default is {@link Color.ExcaliburBlue}
   */
  backgroundColor: Color;

  /**
   * Sets the opacity of the current {@link Graphic} being drawn, default is 1
   */
  opacity: number;
