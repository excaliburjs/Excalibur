...
export class ActorImpl extends Class implements IActionable, IEvented, IPointerEvents, ICanInitialize, ICanUpdate, ICanDraw, ICanBeKilled {
    ...
    */
   set anchor(v: Vector) {
      this.setLocalAnchor(v);
   }

   /**
    * Accessing anchor parameter directly from an Actor automatically converts
    * this anchor to local scope, to prevent changes to global varible.
    */
   get anchor(): Vector {
      this.setLocalAnchor(this._anchor.clone());
   ...
   /**
    * Sets transformations anchor of this Actor to global value;
    */
   public setGlobalAnchor(): void {
      this._anchor = EngineDefaultOptions.getInstance().actors.anchor;
   }

   /**
    * Converts Global scope transformations anchor of this Actor to local value;
    */
   public convertGlobalToLocalAnchor() {
      this.setLocalAnchor(EngineDefaultOptions.getInstance().actors.anchor.clone());
   }

   /**
    * Allows an Actor to have own Local scope transformations anchor;
    * @param anchor A [[Vector]] to be set as Actor's anchor.
    */
   public setLocalAnchor(anchor: Vector): void {
      if (!Vector.isValid(anchor)) {
         return;
      }
   ...
   private _initializeDefaultOptions(): void {
      // default anchor depending on EngineDefaultOptions
      this.setGlobalAnchor();
   }
