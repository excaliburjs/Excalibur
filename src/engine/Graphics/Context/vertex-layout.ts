...
export class VertexLayout {
  ...
  private _attributes: [name: string, numberOfComponents: number][] = []; // TODO the attributes should match the vertex buffer size per vertex at runtime, verify?
  ...
  initialize() {
    ...
    for (const vertAttribute of this._layout) {
      let typeSize = getGlTypeSizeBytes(this._gl, vertAttribute.glType);
