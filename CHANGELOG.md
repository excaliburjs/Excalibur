...
- Breaking change - `Material.update()` now passes a `MaterialContext` to its callback instead of the composite `Shader`. The context carries the material's declarative `uniforms` dictionary (the recommended way to change values — they reach the composite shader and are forwarded to every pipeline pass), plus `shaders`/`shadersByName` for direct access to every compiled shader the material drives. Update code like `material.update(shader => shader.trySetUniformFloatVector('iMouse', pos))` to `material.update(context => context.uniforms.iMouse = pos)`
...
- Fixed issue where `ImageSource.load()` would hang forever below 100% with nothing logged when a response could not be decoded as an image, for example a corrupt image, or a `200 OK` that returned HTML/error content with a content type other than `text/html`. The `HTMLImageElement.onerror` is now handled so `ImageSource.load()` and `ImageSource.ready` reject with an error that is also logged to the console, which lets the `Loader` fail fast instead of deadlocking. `ImageSource.fromHtmlCanvasElement()` no longer hangs when a canvas cannot be turned into a blob, and blob object urls from failed decodes are now released
