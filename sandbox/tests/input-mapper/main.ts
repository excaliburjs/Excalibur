...
actor.onInitialize = (engine) => {
  ...
  ex.Gamepads.MinAxisMoveThreshold = 0.1;
  // Move right
  ...
  mapper.on(
    ({ keyboard, pointers }) => (keyboard.isHeld(ex.Keys.Enter) && pointers.primary.lastWorldPos.x > actor.pos.x ? 1 : 0),
    moveRight
  );
  ...
  mapper.on(
    ({ keyboard, pointers }) => (keyboard.isHeld(ex.Keys.Enter) && pointers.primary.lastWorldPos.y > actor.pos.y ? 1 : 0),
    moveDown
  );
};
game.onPostUpdate = () => {
