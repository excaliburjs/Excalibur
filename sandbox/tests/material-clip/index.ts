...
void main() {
  ...
  vec2 pixelCoord = v_uv.xy * u_resolution.xy;
  float top = u_clip.x;
  float right = u_clip.y;
  float bottom = u_clip.z;
  float left = u_clip.w;
  if ( pixelCoord.x < left || pixelCoord.y < top || pixelCoord.x > right || pixelCoord.y > bottom) {
    discard;
  }
...
var swirlMaterial = game.graphicsContext.createMaterial({
  ...
  fragmentSource,
  uniforms: {
    u_clip: new Float32Array([200, 300, 300, 200])
  }
});
...
void main() {
  ...
  fragColor.rgb = texture(u_noise, v_uv).rgb;
